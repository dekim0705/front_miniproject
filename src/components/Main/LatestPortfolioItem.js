...
const LatestPortfolioItem = () => {
  const [posts, setPosts] = useState([]);

  useEffect(() => {
    const fetchLatest5Portfolio = async () => {
      try {
        const response = await MainAxiosApi.latest5PortfolioPosts();
        setPosts(response.data);
        console.log(response.data);
      } catch (error) {
        console.error("포트폴리오 최근 게시글 불러오기 에러😱", error);
      }
    };
    fetchLatest5Portfolio();
  }, []);
